...
@Component({
  ...
  templateUrl: './generate-dynamic-stock-report.component.html',
  styleUrls: ['./generate-dynamic-stock-report.component.scss']
})
export class GenerateDynamicStockReportComponent implements OnInit   {
  ...
  selectedSupplier2!: Supplier;
  stocksSuppliedBySelectedSupplier!: Stock[];

  onSupplierSelect(supplier: Supplier) {
    this.selectedSupplier2 = supplier;
  ...
  GetAllStock(): void {
    this.stockService.getAllStocks(this.selectedsupp).subscribe({
      next: (stock) => {
        this.stocks = stock;
      },
      error: (response) => {
  ...
  GetAllSuppliers(): void {
    this.supplierService.getAllSuppliers(this.selectedsupp).subscribe({
      next: (suppliers) => {
        this.suppliers = suppliers;
      },
      error: (response) => {
